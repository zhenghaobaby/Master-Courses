...
private:
	...
	static const std::array<unsigned, 12> days_in_month_l;  // num of days in month M in a leap year
	static const std::array<unsigned, 12> days_ytd_l;      // num of days since 1-jan to 1-M in a leap year
...
public:
    ...
    std::string to_string(bool pretty = true) const
    {
		...
		if (m_is_leap == false)
		{
			m_position = lower_bound(days_ytd.cbegin(), days_ytd.cend(), serial_ - days_epoch[m_y - 1900] + 1) - days_ytd.cbegin();
		}
		else
		{
			m_position = lower_bound(days_ytd_l.cbegin(), days_ytd_l.cend(), serial_ - days_epoch[m_y - 1900] + 1) - days_ytd_l.cbegin();
		}
