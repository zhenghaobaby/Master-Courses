...
%     ezplot(smile,[0:0.1:5]);
    ...
    blackcall=getBlackCall(fwd,0.8,fwd,vol(1));
    
    %bs put call parity
    blackput=getBlackPut(fwd,0.8,fwd,vol(1));
    
    if ((u+fwd) - (blackput + fwd)) < 0.0001 && abs(blackcall-u) <0.001
        fprintf('Pass Test')
    else
        fprintf('Test Failed')
    end
